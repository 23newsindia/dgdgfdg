...
add_action('template_redirect', function() {
    if (is_404() && get_option('security_enable_410_redirect')) {
        $excluded_urls_raw = get_option('security_410_excluded_urls', '');
        $excluded_urls = array_filter(array_map('trim', explode("\n", $excluded_urls_raw)));

        $current_path = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);

        foreach ($excluded_urls as $excluded) {
            if (stripos($current_path, $excluded) !== false) {
                return; // normal 404
            }
        }
        ...
        // Fetch and show the content from /security-410/ without redirect
